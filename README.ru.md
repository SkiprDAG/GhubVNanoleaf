...
#### 4. Открытие панели управления и установка PWA:
* **На компьютере:** перейдите по адресу **`http://localhost:8000`** или **`http://nanoleaf.local:8000`**.
* **На телефоне или планшете:** откройте **`http://nanoleaf.local:8000`** в домашней Wi-Fi сети.
* **Установка как приложение (PWA):**
  * В браузере (Chrome / Edge / Safari) нажмите **«Установить приложение»** или **«На экран „Домой“»**.
  * Приложение появится на рабочем столе и будет запускаться в собственном окне без интерфейса браузера как нативный пульт управления.
